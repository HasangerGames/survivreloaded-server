    ...
    writeData(stream) {
        if (!this.p.skipObjectCalculations) {
            for (const object of this.p.map.objects) {
                ...
                const minX = this.p.position.x - cullingRadius, maxX = this.p.position.x + cullingRadius, minY = this.p.position.y - cullingRadius, maxY = this.p.position.y + cullingRadius;
                if (object.position.x >= minX &&
                    ...
                    const index = this.p.visibleObjects.indexOf(id);
                    if (index != -1) {
        ...
        if (this.p.gasCircleDirty)
            valuesChanged += 16;
        if (this.p.game.playerInfosDirty)
            valuesChanged += 32;
        if (this.p.deletedPlayerIdsDirty)
            valuesChanged += 64;
        if (this.p.playerStatusDirty)
            valuesChanged += 128;
        if (this.p.groupStatusDirty)
            valuesChanged += 256;
        if (this.p.bulletsDirty)
            valuesChanged += 512;
        if (this.p.explosionsDirty)
            valuesChanged += 1024;
        if (this.p.emotesDirty)
            valuesChanged += 2048;
        if (this.p.planesDirty)
            valuesChanged += 4096;
        if (this.p.airstrikeZonesDirty)
            valuesChanged += 8192;
        if (this.p.mapIndicatorsDirty)
            valuesChanged += 16384;
        if (this.p.killLeaderDirty)
            valuesChanged += 32768;
        stream.writeUint8(utils_1.MsgType.Update);
        ...
        if (this.p.deletedObjectsDirty) {
            ...
            for (const deletedObject of this.p.deletedObjects)
                stream.writeUint16(deletedObject);
        ...
        if (this.p.game.deletedPlayerIds.length > 0) {
            ...
            for (const id of this.p.game.deletedPlayerIds)
